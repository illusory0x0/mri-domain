...
- **失效会升级**：括号失配本身多是可修复的扰动，但与「模糊匹配 + 无编译闭环 + 大文件」叠加时会升级——从无效重试（同一错误反复修）到震荡（修 A 坏 B），再到**死亡螺旋**（语法破损 → 下游诊断刷屏 → 追错方向 → 上下文被垃圾填满 → 终止，文件可能停在不可逆的半破损态）。这是端到端成本被放大的主要路径，也是结构化编辑最能兑现价值之处：从源头挡掉非法中间态。
- **解锁小模型 / 小上下文**：当前要让文本编辑 agent 可靠，只能靠「大模型 + 大上下文 + verify/retry」硬扛——慢且贵。本方案把**正确性下沉到工具、把「理解」变成查询**（§1.4 的 symbol query / 结构化 viewer）：模型不必读入整份文件，也不必保证括号正确，于是**更小、更便宜的模型与更小的上下文即可胜任**。前提是接口足够高层（code action 优先于 primitive，见 §1.6）；若把界面做成一长串 primitive，反而抬高对模型推理的要求，与「降档」目标相悖。
